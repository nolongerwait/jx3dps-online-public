...
async function 获取全部部位的数据(本次导入功法) {  
  // 非武器获取装备
  Object.keys(装备部位枚举).filter(key => key !== "武器" && key !== "重剑").forEach(async key => { 
    ...
    if (res?.length) {
      ...
      console.info(`【${本次导入功法}】【${key}】数据获取失败`)
    }
  })

   // 判断藏剑专属重剑逻辑
  const 是否藏剑 = 本次导入功法 === '外功'  
  const 武器部位 = 是否藏剑 ? "重剑" : "武器"
 ...
 const [紫武PVE数据, 紫武PVX数据, 橙武数据] = await Promise.all([
  ...
  const 紫武排序数据 = 装备排序((紫武PVE数据 || []).concat(紫武PVX数据 || []))
  const 橙武排序数据 = 装备排序(橙武数据 || [])

  const 结果数据 = 橙武排序数据.concat(紫武排序数据)
  if (结果数据?.length) {
    导出成文件(结果数据, 武器部位, 本次导入功法)
  } else {
    console.info(`【${本次导入功法}】【${武器部位}】数据获取失败`)
  }
}
