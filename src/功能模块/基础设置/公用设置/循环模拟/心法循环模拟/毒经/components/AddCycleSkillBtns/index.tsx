  ...
  return (
    <div className={'cycle-simulator-setting-btns'}>
      <div className={'cycle-simulator-setting-item'}>
        <span className={'ztg-cycle-btn-type'}>千劫万毒手</span>
        <Space className={'cycle-simulator-setting-skills'} size={[8, 16]} wrap>
          {模拟信息?.技能基础数据
            ?.filter((item) => {
              return !item?.创建循环不可选 && item?.技能类型 === '千劫万毒手'
            })
            .map((item) => {
              return (
                <AddCycleSkillBtn
      ...
      <div className={'cycle-simulator-setting-item'}>
        <span className={'ztg-cycle-btn-type'}>引魂蛊术</span>
        <Space className={'cycle-simulator-setting-skills'} size={[8, 16]} wrap>
          {模拟信息?.技能基础数据
            ?.filter((item) => {
              return !item?.创建循环不可选 && item?.技能类型 === '引魂蛊术'
            })
            ?.filter((item) => {
              if (item?.显示类型 === '奇穴技能') {
      ...
      <div className={'cycle-simulator-setting-item'}>
        <span className={'ztg-cycle-btn-type'}>驭虫奇术</span>
        <Space className={'cycle-simulator-setting-skills'} size={[8, 16]} wrap>
          {模拟信息?.技能基础数据
            ?.filter((item) => !item?.创建循环不可选 && item?.技能类型 === '驭虫奇术')
            .map((item) => {
              return (
                <AddCycleSkillBtn
                  onClick={() => 新增循环技能(item)}
      ...
      {/* <div className={'cycle-simulator-setting-item'}>
        <span className={'ztg-cycle-btn-type'}>达摩武诀</span>
        <Space className={'cycle-simulator-setting-skills'} size={[8, 16]} wrap>
          {模拟信息?.技能基础数据
            ?.filter((item) => !item?.创建循环不可选 && item?.技能类型 === '达摩武诀')
            .map((item) => {
              return (
                <AddCycleSkillBtn
                  onClick={() => 新增循环技能(item)}
        ...
        <Space className={'cycle-simulator-setting-skills'} size={[8, 16]} wrap>
          {模拟信息?.技能基础数据
            ?.filter((item) => {
              ...
              if (item?.显示类型 === '奇穴技能') {
                const 依赖名称 = item?.依赖奇穴名 || item?.技能名称
                return 奇穴信息?.includes(依赖名称) && item?.技能类型 === '其他'
              }
