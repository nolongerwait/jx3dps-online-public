...
const AddCycleSkillBtn: React.FC<AddCycleSkillBtnProps> = (props) => {
  ...
  const 异常信息: 异常信息数据 = useMemo(() => {
    ...
    const 橙武buff = 模拟信息?.当前自身buff列表?.['橙武']
    const 当前时间 = 模拟信息?.当前时间
    ...
    } else if (技能?.技能名称 === '千里急' && 模拟信息?.角色状态信息?.状态 !== '傀儡') {
      return {
        ...禁用信息,
        是否禁用: true,
        异常描述: ERROR_ACTION?.状态错误?.信息,
      }
    } else if (
      技能?.技能名称 === '夹线' &&
      (!橙武buff || 当前时间 - (橙武buff?.刷新时间 || 0) > (橙武buff?.最大持续时间 || 0))
    ) {
      return {
        ...禁用信息,
        是否禁用: true,
