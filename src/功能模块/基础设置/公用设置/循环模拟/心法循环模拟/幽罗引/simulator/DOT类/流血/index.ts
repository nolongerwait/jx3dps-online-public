...
class 流血 extends 通用DOT类 {
  ...
  获得和刷新流血() {
    ...
    this.击水判定(当前流血层数)
    const 流血最大层数 = this?.模拟循环?.Buff和Dot数据?.流血?.最大层数 || 2
    const 添加流血层数 = 1
  ...
  获得破风() {
    this.模拟循环.添加buff?.({ 名称: '破风', 对象: '目标' })
  }

  击水判定(当前流血层数) {
    if (this.模拟循环.校验奇穴是否存在('击水') && 当前流血层数 >= 3) {
      this.触发伤害行为('击水')
    }
  }

  结算流血伤害(事件时间 = this.模拟循环.当前时间) {
    ...
    待生效数据?.forEach((数据) => {
      ...
      if (生效时间) {
        this.触发伤害行为('流血(DOT)', 1, 快照buff列表, 生效时间, 层数)
      }
    })
  }
}
