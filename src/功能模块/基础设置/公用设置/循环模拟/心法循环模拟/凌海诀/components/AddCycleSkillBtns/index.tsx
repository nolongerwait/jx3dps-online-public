  ...
  return (
    <div className={'cycle-simulator-setting-btns'}>
      <div className={'cycle-simulator-setting-item'}>
        <span className={'lhj-cycle-btn-type'}>风起云扬诀</span>
        <Space className={'cycle-simulator-setting-skills'} size={[8, 16]} wrap>
          {模拟信息?.技能基础数据
            ?.filter((item) => {
              return !item?.创建循环不可选 && item?.技能类型 === '风起云扬诀'
            })
            .map((item) => {
              return (
                <AddCycleSkillBtn
                  onClick={(extra) => 新增循环技能(item, extra)}
                  key={item?.技能名称}
                  className={'lhj-cycle-simulator-setting-btn'}
                  完整循环={处理循环结果对象?.完整循环 || []}
                  技能={item}
                  模拟信息={模拟信息}
                  插入技能={插入技能}
      ...
      <div className={'cycle-simulator-setting-item'}>
        <span className={'lhj-cycle-btn-type'}>长空破云式</span>
        <Space className={'cycle-simulator-setting-skills'} size={[8, 16]} wrap>
          {模拟信息?.技能基础数据
            ?.filter((item) => {
              return !item?.创建循环不可选 && item?.技能类型 === '长空破云式'
            })
            ?.filter((item) => {
              if (自动鸟啄) {
                return item?.技能名称 !== '翼绝云天'
              } else {
                return true
              }
            })
      ...
      <div className={'cycle-simulator-setting-item'}>
        <span className={'lhj-cycle-btn-type'}>九霄踏云步</span>
        <Space className={'cycle-simulator-setting-skills'} size={[8, 16]} wrap>
          {模拟信息?.技能基础数据
            ?.filter((item) => !item?.创建循环不可选 && item?.技能类型 === '九霄踏云步')
            .map((item) => {
              return (
                <AddCycleSkillBtn
                  onClick={(extra) => 新增循环技能(item, extra)}
      ...
      <div className={'cycle-simulator-setting-item'}>
        <span className={'lhj-cycle-btn-type'}>碧海缥缈掌</span>
        <Space className={'cycle-simulator-setting-skills'} size={[8, 16]} wrap>
          {模拟信息?.技能基础数据
            ?.filter((item) => !item?.创建循环不可选 && item?.技能类型 === '碧海缥缈掌')
            .map((item) => {
              return (
                <AddCycleSkillBtn
                  onClick={(extra) => 新增循环技能(item, extra)}
      ...
      <div className={'cycle-simulator-setting-item'}>
        <span className={'lhj-cycle-btn-type'}>其他</span>
        <Space className={'cycle-simulator-setting-skills'} size={[8, 16]} wrap>
          {模拟信息?.技能基础数据
            ?.filter((item) => {
      ...
      <div className={`cycle-simulator-setting-item cycle-simulator-setting-fast-item`}>
        <span className={'lhj-cycle-btn-type'}>快捷添加</span>
