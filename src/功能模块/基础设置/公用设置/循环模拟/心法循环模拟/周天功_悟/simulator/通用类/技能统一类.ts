...
class 技能统一类 {
  ...
  技能回复能量(回复数值 = 20, 能量类型: '任脉' | '督脉' | '任督' = '任脉') {
    if (this.模拟循环.角色状态信息) {
      if (['任脉', '督脉'].includes(能量类型)) {

        this.模拟循环.角色状态信息.能量信息[能量类型] = Math.min(
          this.模拟循环.角色状态信息.能量信息[能量类型] + 回复数值,
          100,
        )
      } else {
        if (this.模拟循环.角色状态信息.能量信息.任脉 < this.模拟循环.角色状态信息.能量信息.督脉) {
          this.模拟循环.角色状态信息.能量信息.任脉 = Math.min(
            this.模拟循环.角色状态信息.能量信息.任脉 + 回复数值,
            100,
          )
        } else {
          this.模拟循环.角色状态信息.能量信息.督脉 = Math.min(
            this.模拟循环.角色状态信息.能量信息.督脉 + 回复数值,
            100,
          )
        }
      }
  ...
  是否经脉循行() {
    if (this.模拟循环.角色状态信息) {
      return this.模拟循环.角色状态信息.能量信息.任脉 >= 50 && this.模拟循环.角色状态信息.能量信息.督脉 >= 50
    }
  }
