...
class 没石饮羽 extends 技能统一类 {
  ...
  释放没石饮羽() {
    ...
    const 待生效事件: 待生效事件[] = []
    let 能否触发于狩 = false
    let 最后一箭能否触发棘矢 = false

    if (this.模拟循环.校验奇穴是否存在('于狩')) {
      this.模拟循环.获得承契()
      const 剩余箭数 = this.模拟循环.角色状态信息.箭袋信息?.length
      能否触发于狩 = 剩余箭数 <= 5 && 剩余箭数 >= 3
      最后一箭能否触发棘矢 = 剩余箭数 === 3
    }
    ...
    for (let i = 0; i < 没石饮羽.作用次数; i++) {
      ...
      待生效事件.push({
        ...
        事件备注: {
          ...
          最后一箭触发棘矢: 最后一箭能否触发棘矢,
          次数: i + 1,
          当前箭状态: this.模拟循环.角色状态信息.箭袋信息[i]?.箭形态,
        },
      })
    }
  ...
  没石饮羽触发(配置) {
    ...
    // 3箭释放时最后一箭触发棘矢
    if (配置.最后一箭触发棘矢 && 配置.次数 === 3) {
      this.模拟循环.棘矢引爆贯穿判定(`没石饮羽`)
    }
    this.模拟循环.技能类实例集合?.饮羽簇?.饮羽簇造成伤害?.('没石饮羽', 配置)
  }
