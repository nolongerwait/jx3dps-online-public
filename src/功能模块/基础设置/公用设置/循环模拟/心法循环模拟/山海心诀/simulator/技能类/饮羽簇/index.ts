...
class 饮羽簇 extends 有CD技能通用类 {
  ...
  constructor(模拟循环) {
    ...
    饮羽簇.技能数据 = 模拟循环?.技能基础数据?.find((item) => item.技能名称 === '饮羽簇')

    // 根据加速修改实际读条帧
    const 循环加速值 = this.模拟循环.加速值 || 0
    饮羽簇.读条时间 = 获取实际帧数(this.判断饮羽簇读条时间(), 循环加速值)
  ...
  获取饮羽簇冷却() {
    const 秘籍减冷却 = 判断秘籍减冷却('饮羽簇', this.模拟循环.秘籍)
    let 技能原始CD = (每秒郭氏帧 * 10 || 0) - 秘籍减冷却
    if (this.模拟循环.校验奇穴是否存在('丛云隐月')) {
  ...
  释放() {
    ...
    // 释放饮羽充值白羽流星
    if (this.模拟循环.秘籍?.['饮羽簇']?.includes('重置白羽')) {
  ...
  饮羽簇造成伤害(类型?, 配置?) {
    if (类型 !== '没石饮羽') {
      this.模拟循环.棘矢引爆贯穿判定(类型 || '饮羽簇')
    }
    this.模拟循环.金乌箭判定(配置?.当前箭状态)
    this.模拟循环.标鹄判定()
    this.触发伤害行为('饮羽簇')
  ...
  读条伤害() {
    // 正读条引在读条结束后才进入CD
    if (饮羽簇.技能数据) {
      this.模拟循环?.技能释放后更新运行数据?.(饮羽簇.技能数据, this)
    }
    this.饮羽簇造成伤害()
  }
